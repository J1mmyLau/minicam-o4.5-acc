...
## 2026-08-14 05:00 | OFFICIAL_RTF_RESOLVED | **RTF 阻塞解除（首次实测数值）**。根因非「发射缺失」，而是 LISTEN-wedge 生命周期 bug：自然 LISTEN chunk 经 tts_mark_producer_done q==0 路径推进所有 drain 计数器后，仍有一条冗余空 is_chunk_end=true/is_final=false T2W 任务入队，worker 置 active_t2w_generation=gen 却无 reset（LISTEN 代既非 is_final 也非 EOS-empty），外层 omni_duplex_drain_tts_audio 读 active_gen==my_gen → NOT_REUSABLE → 拒绝后续所有 decode → 0 SPEAK。修复 = 2 处外科手术 edit（tools/omni/omni.cpp，非受保护）：(1) 空 duplex LISTEN chunk_end 不置 active；(2) 新增完成 no-op 块幂等推进 final_processed/final_vocoder + 清零 active + notify。官方 RTS 重跑（Config D，omni_duplex1.mp4，gpu 1）：core.rtf_aggregate run1=1.0904 / run2=1.1653（full 0.9821 / 1.0849），n_speak 0→33（2 次均 33），0 拒绝，stage_timing.jsonl present。与 baseline 1.087 parity（core 样本仅 19 帧，run-to-run ~7% 噪声）。二进制 server 4694cb58… / libomni 3f3e1e63…。状态：OFFICIAL_PERFORMANCE_GATE=PASS，RTF_BLOCKER_CLASS=A→RESOLVED。见 docs/F6_OFFICIAL_RTF_RESOLVED.md。
