...
## 2026-08-05 | DOCS | F6_DOCUMENTATION_EXPANSION_COMPLETE
Task #354 完成。顶层文档 3→8 份（README/QUICKSTART/ARCHITECTURE/OPTIMIZATION_AND_RESULTS/METHODOLOGY/REPRODUCTION_GUIDE/LIMITATIONS_AND_OFFICIAL_GATES/EVIDENCE_INDEX），总量 2,276 行。质量检查全部 PASS：git diff --check clean / 14 evidence path 全部存在 / 无 over-claim / FINAL_INTERNAL+NOT_RUN+BLOCKED_BY_OFFICIAL_STARTER_KIT+NOT_CLAIMED 跨文档一致。冻结源码 bdd4550 未修改。

## 2026-08-05 | AUDIT | CANN_AUDIT_CONSISTENCY_FIX — 审计一致性修正（用户指令）：①MAIN_LLM_RUNTIME_PLACEMENT 降级（PASS→PARTIAL）：同时写了 RUNTIME_PLACEMENT=PASS 和 GRAPH_SPLIT_RUNTIME_COUNT=NOT_MEASURED / RUNTIME_REACHED=NOT_MEASURED / FALLBACK_COUNT=NOT_MEASURED 构成矛盾；"未观察到 CPU fallback"≠"运行时 Placement PASS"；改为四级状态（STATIC_CAPABILITY=PASS / STATIC_PLACEMENT=PASS / RUNTIME_PLACEMENT=PARTIAL / CPU_FALLBACK_OBSERVED=NO）。②−81.4% 补完整指标定义（13 字段：metric/baseline/candidate/Δ/n/CI95/commit/hardware/evidence raw/report/status/适用范围），明确 HISTORICAL_INTERNAL_RESULT，非官方 chunk RTF/非全请求 E2E/非 vLLM。修改文件：docs/audit/CANN_CPU_NPU_PLACEMENT_AUDIT.md、docs/F6_OPTIMIZATION_AND_RESULTS.md、docs/F6_README.md、docs/F6_LIMITATIONS_AND_OFFICIAL_GATES.md、STATUS.md、memory/f6-cann-placement-audit.md。冻结源码 bdd4550 未修改。
...
## 2026-08-05 | COMPETITION | GATE_TOOLING_CLOSURE — 收口指令执行（P0/P1 四修复 + 资产标签 + 就绪度报告更新）。①`run_performance.sh` MODE=baseline|candidate（同 runner/数据/seed/warmup/count/统计，仅 MODE 切配置），输出隔离 `${OUTPUT_ROOT}/<run_id>/<mode>/`，每次落 manifest.json（resolved config/完整启动命令/完整 benchmark 命令/source commit/binary SHA/model SHA/data SHA/env），新增 `check_baseline_candidate_symmetry.py`（比对 data_sha/case count/request_ids/sampling/model/prompt/stats_code_sha，不一致退出非零；fixture 实测 0/1/2）。②`analyze_chunk_rtf.py` valid_audio 真实判定（10 排除原因枚举 EMPTY_PAYLOAD/ZERO_SAMPLES/INVALID_SAMPLE_RATE/DECODE_FAILURE/NAN_INF/MISSING_REQUEST_ID/MISSING_CHUNK_INDEX/INVALID_TIMESTAMP/DUPLICATE_CHUNK/TRUNCATED_CHUNK，删恒 true 桩），summary 含 total/valid/invalid/exclusion_rate/reason_counts；冻结 T6 日志实测 329 chunk 全 valid p50=0.2784；单测 21 例。③4 份 Gate 脚本显式 `--dry-run`（返回码 0=就绪/2=缺官方资产或 Harness/3=配置非法/4=baseline/candidate 不对称；不起服务不占 NPU不发请求不落成绩）。④私有默认路径清除：MODEL_PATH 必填无默认、DEMO_DIR/DATA_ROOT/OUTPUT_ROOT/OFFICIAL_HARNESS_ROOT 从 REPO_ROOT 派生；`check_no_private_paths.py` 审计 PASS。资产标签：LOCAL_ASSET_MANIFEST=PASS / OFFICIAL_ASSET_VERSION_MATCH=PENDING_STARTER_KIT（当前 SHA 仅 CURRENT_LOCAL_ASSET_SNAPSHOT）。就绪度报告状态更新为 DRY_RUN_SUPPORT=BASELINE_CANDIDATE_SYMMETRY=CHUNK_AUDIO_VALIDATION=PRIVATE_PATH_AUDIT=LOCAL_ASSET_MANIFEST=PASS / OFFICIAL_ASSET_VERSION_MATCH=PENDING_STARTER_KIT / OFFICIAL_GATES=BLOCKED_BY_OFFICIAL_STARTER_KIT。新增 `submission/tests/`（test_analyze_chunk_rtf.py + make_symmetry_fixture.py + check_no_private_paths.py + run_selftest.sh）与 `docs/competition-submission/OFFICIAL_GATE_TOOLING_SELFTEST.md`（14/14 PASS）。OFFICIAL_GATE_TOOLING_READINESS=PASS；COMPETITION_COMPLETE=NOT_CLAIMED。冻结源码 bdd4550 未动，未运行正式 Benchmark，未填官方结果。

## 2026-08-05 | AUDIT | CANN_CPU_NPU_PLACEMENT_AUDIT — 只读旁路审计完成（任务#352，用户指令）：输出 docs/audit/ 5 份（主审计 / supports_op 矩阵 / 同步与拷贝 / KV Cache 放置 / Per-Chunk RTF 相关性）。源码覆盖：ggml-cann.cpp（完整 supports_op 60+ op + offload_op + sync/copy 模式 + caps.async=false + ACL graph 机制）、ggml-backend.cpp（scheduler split 分配 5-pass + split 间同步/拷贝/event）、llama-context.cpp（offload_kqv=true + pipeline_parallel=false 因 caps.async=false）、llama-kv-cache.cpp（多 backend buffer 共存）、omni.cpp（3 独立 llama context + TTS 独立 GPU layers + projector backend）、token2wav-impl.cpp（vocoder CANN 分发计数）。核心发现：caps.async=false → 无 pipeline parallel；offload_op ne[1]>=32 gate 对 decode bs=1 始终 false 但不影响 -ngl 999；split 间同步 p50=0ms（CANN stream 通常已完成）；D2H 读取 logits/hidden 为语义必须；multi-die D2D 后必须 aclrtSynchronizeStream。冻结源码 bdd4550 未修改，未运行 Benchmark，未填写官方结果。CANN_PLACEMENT_AUDIT=PASS。

## 2026-08-05 | AUDIT | CANN_AUDIT_EVIDENCE_CORRECTION — 审计证据层级修订（任务#353，用户指令）：10 项修正。①三级状态拆分：CANN_STATIC_CAPABILITY_AUDIT=PASS / MAIN_LLM_RUNTIME_PLACEMENT=PASS / CPU_PER_CHUNK_CRITICAL_PATH=TO_MEASURE。②mutex_wait(p50=0ms)≠stream sync 纠正：前者=线程锁/条件变量等待，后者=Host等CANN Stream完成，测不同东西；STREAM_SYNC_SOURCE_PATH=CONFIRMED / STREAM_SYNC_RUNTIME_COST=NOT_MEASURED。③静态 supports_op ≠ 运行时可达：矩阵补 STATIC_SUPPORTED/CONDITIONS/TARGET_GRAPH_USES_OP/RUNTIME_REACHED/OBSERVED_BACKEND/FALLBACK_COUNT/PER_CHUNK_RELEVANCE 列，冻结模型所有条件性 fallback 标记 NOT_MEASURED。④Tensor 放置措辞修正：由-ngl 999控制的主模型可卸载weight tensor在CANN≠全部tensor在CANN；input/index/control/metadata/output 可能 CPU/Host，分 6 类列明。⑤Graph split：静态分析 decode bs=1 几乎不分裂，运行时 NOT_MEASURED（GGML_SCHED_DEBUG=1 需模型文件）。⑥逐chunk时间预算：G7日志 797 chunks 解析（inference p50=227ms / RTF p50=0.23 / queue_wait p50=0ms）；补全 12 项逐chunk预算表，10项标记 NOT_MEASURED。⑦版本化历史结论：历史基线(3f7a7f0/CANN 9.0/CPU T2W) vs 冻结候选(bdd4550/CANN 9.1.0-beta.1/CANN Flow+Vocoder) 显式区分。⑧KV Cache 数量修正：2 套独立 llama_context (LLM+TTS)，非 3 套；Talker 复用主 LLM KV cache。⑨D2H 数据补全：D2H_EXISTS=CONFIRMED / D2H_COST=NOT_MEASURED；标注函数/tensor/方向/调用频次未知。⑩最终状态推荐 11 项。历史 msprof 数据仅作存在性参考（旧 commit 2026-07-28，46,914 次 SynchronizeStream p50=1.1μs / 25,226 次 sync Memcpy p50=25.1μs，不等同于冻结候选）。5 份文档全部修订。冻结源码 bdd4550 未修改。FROZEN_SOURCE_UNCHANGED=YES。
