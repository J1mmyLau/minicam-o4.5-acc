...
| T15 | **DONE (20260804)** | 冻结源码重建二进制上的 T6 完整重跑（任务#334 最后一步，指令#3/#8）— 冻结源码 bdd4550 → 两次干净重建 SHA 一致（REPRODUCIBLE_BINARY=PASS: libomni `c4b16937` / server `db258375`）→ T6 在冻结二进制上重跑 **11/11 GATES PASS, ACCEPT=True**（meta.binary_sha=db258375）：S13 120/120（err=0 eos=81/max_tokens=39, decode_wall_p50=5437ms）；Extended 30/30（long 20+mixed 10）；Voice 5/5+isolation（5 distinct）；Disconnect 5/5+followup；KV A/B 30pairs(valid 28, MISS 202.8→HIT 82.0ms Δ121.2 2.47×)；Restart 3 会话；smoke 5/5；cpu_fallback=0 cann_error=0 cann_ok=4。user_text P0 修复触及 media_type=1 音频路径已覆盖。**POST_T11_SOURCE_FREEZE=IN_PROGRESS→PASS**，**POST_T11_FINAL_CANDIDATE→FINAL_INTERNAL**。8 项指令全部完成；官方 Gate（OFFICIAL_ACCURACY/BENCHMARK）在官方 Harness 通过前一律 NOT_CLAIMED。证据 `docs/f6-s13-closure/phase2/t6_integrated_regression.json`（binary_sha=db258375）。 |
| T16 | **DONE (20260805)** | 比赛收口阶段（用户 12 节收口 Prompt + vLLM 迁移对齐指令）— ①Phase A（任务#336）：`docs/competition-submission/` 10 份（需求矩阵/门状态/benchmark 计划/Demo 计划/chunk RTF 规格/性能报告模板/repro 审计/提交清单/Demo 指南/视频脚本）；②Phase B（任务#337）：`submission/` 提交包骨架 30 文件，脚本 `set -Eeuo pipefail` + check deps/model/NPU/port + run_id/命令/env/raw 落盘 + fail 非零 + 无 /tmp 默认，Demo 脚本 + DEMO_USER_GUIDE + DEMO_VIDEO_SCRIPT；③**逐 chunk RTF 离线采集链路**（冻结二进制日志已含 `RTF=0.23`，`submission/scripts/analyze_chunk_rtf.py` 解析，无需改源码）；④VLLM_ALIGN（任务#340）：新增 `VLLM_METRIC_MEASUREMENT_SPEC.md` + 7 份对齐比赛约束层（§2.5 映射/组件 17 字段/执行计划比赛口径 V0–V12/R26–R40/交接包准入优先+第一周禁止清单/5 比赛模板）。状态：`COMPETITION_CLOSURE_DOCS=DONE`、`VLLM_MIGRATION_COMPETITION_ALIGN=DONE`、官方 Gate 全部 `NOT_RUN (BLOCKED_BY_OFFICIAL_STARTER_KIT)`、`COMPETITION_COMPLETE=NOT_CLAIMED`。**冻结纪律**：bdd4550 性能源码未动，仅新增文档/脚本。两提交：`docs(competition)` + `docs(vllm-migration)`。 |
