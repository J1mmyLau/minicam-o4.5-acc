...
> 比赛风险（R26–R40）用 5 字段：触发条件 / 检测指标 / Gate 影响 / 处理方案 / 回归用例（口径见 `VLLM_METRIC_MEASUREMENT_SPEC.md`）。
...
## 2. 重点风险矩阵（25 条 × 9 字段 + 比赛风险 R26–R40 × 5 字段）

> 状态 owner 命名：`V3/V4/…`（执行计划阶段）或 `官方资产/队友`。风险 1–16 源自 llama 实测教训；17–25 为 vLLM 侧特有风险（TO_AUDIT）；26–40 为比赛风险（准入/排名/提交口径）。
...
## 2.5 比赛风险清单（R26–R40，5 字段）

> 比赛风险字段：触发条件 / 检测指标 / Gate 影响 / 处理方案 / 回归用例。新增于 2026-08-05（比赛约束层）。

### R26 — 精度降幅 >2pp 被准入拒绝

- **触发条件**：V9/V10/V11 任一基准相对官方基线降幅 >2pp。
- **检测指标**：Daily-Omni / TTS-Seed / Video-MME 的 candidate−baseline 差值（绝对值）。
- **Gate 影响**：准入失败 → 排名无效（最高级别阻断）。
- **处理方案**：回退最近一次性能优化并重测；检查 packing/媒体路径；确认与官方基线**同口径**对比（只测 candidate 未测 baseline 见 R27）。
- **回归用例**：V9/V10/V11 全量重跑。

### R27 — 只测 candidate 未测官方 baseline

- **触发条件**：报告只有绝对分数、没有同口径官方基线对比。
- **检测指标**：报告缺"相对官方基线降幅"字段（accuracy_comparison 模板 §7）。
- **Gate 影响**：2pp 准入无从计算 → 评审质疑。
- **处理方案**：官方 Starter Kit 到达后补跑 baseline；此前所有精度数字标"内部口径（官方基线未定）"。
- **回归用例**：accuracy_comparison.md 模板补全后重跑。

### R28 — TTFT 口径错误（起点/终点与官方脚本不一致）

- **触发条件**：内部起点用服务端接收、官方用客户端发出（或反之）；终点"首个 token"判定标准不同。
- **检测指标**：TTFT 明显偏小/偏大；与官方样本偏差超预期。
- **Gate 影响**：排名指标数值失真。
- **处理方案**：按指标规范 §3 校准起止事件；标注口径；官方脚本到达后重测。
- **回归用例**：V2 baseline 校准后重跑。

### R29 — TTFP 空包/静音包当有效包

- **触发条件**：首个"数据包"是空包/静音/解码失败包。
- **检测指标**：TTFP 数值异常小；WAV 解码校验失败。
- **Gate 影响**：TTFP 假低分 → 排名失真。
- **处理方案**：TTFP 终点改为"首个**有效**音频包"（指标规范 M5）；加 WAV 有效性断言。
- **回归用例**：V2/V6 TTFP 重测。

### R30 — request RTF / 全请求 RTF 冒充 chunk RTF

- **触发条件**：用客户端总耗时/总音频时长，或用 Flow/Vocoder 内部 RTF 算 RTF。
- **检测指标**：RTF 含网络/排队/解码调度成分，与逐 chunk 口径不一致。
- **Gate 影响**：排名**核心**指标失真（llama 子赛道为唯一排名指标）。
- **处理方案**：逐 chunk 采集（指标规范 §4）；禁止 request/全请求口径作为排名值。
- **回归用例**：`submission/scripts/analyze_chunk_rtf.py` 复跑（llama 侧）/ vLLM 侧打点重算。

### R31 — 首/中/尾 chunk 混统计

- **触发条件**：chunk RTF 统计把首 chunk（冷启动/prefill）与中间/尾混在一起。
- **检测指标**：chunk RTF p50 被首 chunk 拉偏；分桶后差异显著。
- **Gate 影响**：排名数值偏高/失真。
- **处理方案**：三桶分开统计（指标规范 §5）；报告必须给分桶。
- **回归用例**：V2/V7 分桶统计。

### R32 — Benchmark 过但 Demo 不可用

- **触发条件**：只跑 benchmark 未跑官方 Demo。
- **检测指标**：Demo 用例 D1–D12 未记录或失败。
- **Gate 影响**：准入第二门槛失败（精度过了也白过）。
- **处理方案**：V11 强制 Demo 全用例；D1–D12 逐项记录 + 录像留证。
- **回归用例**：demo_validation.md（模板 §8）。

### R33 — 多卡结果当单卡成绩

- **触发条件**：优化实验跑在 2/3 卡 YAML，比赛单卡。
- **检测指标**：报告数值未标注单卡复验。
- **Gate 影响**：官方评测环境与报告不符 → 数值无效。
- **处理方案**：任何收益必须单卡复验；口径文档强制单卡基线。
- **回归用例**：V2/V12 单卡全集。

### R34 — 数据/资产版本错误（qa.json 版本、allowlist、reference audio 版本）

- **触发条件**：官方 benchmark 使用的数据/资产与规则指定版本不符。
- **检测指标**：结果与官方样本差异大；文件 hash 不符。
- **Gate 影响**：精度/失败率失真，可能被判无效。
- **处理方案**：记录数据版本 + SHA；用官方 allowlist；V0 冻结数据清单。
- **回归用例**：V9/V10/V11 全量重跑（版本核对）。

### R35 — 提交依赖 /tmp 或私有绝对路径

- **触发条件**：提交脚本写死 /tmp 持久化或私有绝对路径作为唯一默认值。
- **检测指标**：clean env 跑提交脚本失败。
- **Gate 影响**：工程复现审查失败 → 提交被否。
- **处理方案**：提交脚本全相对路径 + 依赖声明；无 /tmp 持久化（沿用 llama 提交包纪律）。
- **回归用例**：clean env 一键复现（`docs/competition-submission/REPRODUCTION_AUDIT.md`）。

### R36 — SHA 不一致（提交二进制/镜像与声称不符）

- **触发条件**：声称的 server/model/镜像 SHA 与实际提交不符。
- **检测指标**：sha256sum 比对失败。
- **Gate 影响**：复现审查失败 → 成绩无效。
- **处理方案**：VERSION_MANIFEST 记录所有 SHA；提交前核对（`submission/VERSION_MANIFEST.md`）。
- **回归用例**：构建后 SHA 比对（`submission/scripts/build.sh`）。

### R37 — 无法 clean env 复现

- **触发条件**：依赖/安装步骤缺失，或依赖源不可达。
- **检测指标**：从零安装后起服失败。
- **Gate 影响**：工程复现审查失败。
- **处理方案**：requirements.txt 固化 + 安装文档步骤化；镜像 tag 固定。
- **回归用例**：clean env 一键路径（REPRODUCTION_AUDIT）。

### R38 — 把内部 pilot 写成官方 Gate 成绩

- **触发条件**：报告把内部跑分/内部 pilot 标成 OFFICIAL 或"官方通过"。
- **检测指标**：状态标签混用；OFFICIAL_* 在官方脚本未跑时置位。
- **Gate 影响**：信任崩塌 → 全部结果被质疑。
- **处理方案**：状态字面量强制（Handoff §7）；报告必带口径标注。
- **回归用例**：提交前状态核对。

### R39 — 官方权重/归一化未知时自行定权重

- **触发条件**：官方文档未公布权重时，报告自算综合分。
- **检测指标**：报告出现自定义加权总分。
- **Gate 影响**：评审不认 → 排名依据无效。
- **处理方案**：权重未定一律"待官方确认"；只报单项原始分布。
- **回归用例**：报告模板强制字段（指标规范 §8）。

### R40 — Demo 视频/文档缺失或与实际不符

- **触发条件**：提交包缺 Demo 视频/使用说明，或视频与实际行为不符。
- **检测指标**：提交清单缺 demo 产物；视频无法复现。
- **Gate 影响**：交付物不完整被扣分。
- **处理方案**：V11 录像 + DEMO_USER_GUIDE 同步；提交清单逐项勾。
- **回归用例**：`docs/competition-submission/FINAL_SUBMISSION_CHECKLIST.md` 逐项核对。
...
| 全双工 | **DEFER**（附加实验；2026-08-05 起不再是主线 V11，不得阻塞 Simplex 主线） | §17 / 执行计划"附加实验" |
