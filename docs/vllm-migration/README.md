...
```text
先测量，后优化。
第 1 步 = Stage 打点（确认各段占比）
第 2 步 = 设备放置审计（Flow/Vocoder 在 CPU 还是 NPU）
第 3 步 = Prefix Cache A/B（固定前缀是否真正复用）
```
...
| 顺序 | 文件 | 你得到什么 |
|---|---|---|
| 1 | `LLAMA_TO_VLLM_EXPERIENCE_MIGRATION.md` | 12 条核心经验（每条 10 点）+ 4 条决策树 + 请求路径图 + 打点事件表 |
| 2 | `LLAMA_RAW_EVIDENCE_APPENDIX.md` | 所有 llama 数字的精确出处（数值/CI95/样本/来源） |
| 3 | `LLAMA_VLLM_COMPONENT_MAPPING.md` | 13 个组件逐一映射 + 6 条 rg 源码导航命令 |
| 4 | `VLLM_OPTIMIZATION_EXECUTION_PLAN.md` | 动手路线 V0–V12，每阶段 16 字段 |
| 5 | `VLLM_RISK_AND_VALIDATION_MATRIX.md` | 16 条证据 + 25 条风险（每条 9 字段）+ 候选决策 |
| 6 | `VLLM_TEAM_HANDOFF.md` | 队友第一周计划 + 第一个小时该做的 5 件事 + 最终交付清单 |
| 7 | `EXPERIMENT_TEMPLATES.md` | 4 种实验模板（Run Manifest / Per-request / 决策 / A/B 清单） |

> 快速路径：**先读 1 的 §0–§5**，遇到数字查 2，定位代码查 3，动手按 4，出故障按 5。
...
```text
基线（V2 单卡） → 占比（V3 打点） → 设备（V4 审计） → 复用（V5 Prefix Cache A/B）
```

本周不写算子，只建立四张地图。详见 `VLLM_TEAM_HANDOFF.md` §3。
