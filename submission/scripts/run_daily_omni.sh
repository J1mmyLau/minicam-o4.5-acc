...
DATA="${DATA:-${DATA_ROOT}/Daily-Omni}"
RUN_ID="${RUN_ID:-run_$(date +%Y%m%d_%H%M%S)_daily_omni}"
...
[ -f "${REPO_ROOT}/submission/config/benchmark.yaml" ] || MISSING+=("benchmark.yaml")
HARNESS_MISSING=0
...
# ================= DRY-RUN =================
if [ "${DRY_RUN}" = "1" ]; then
  echo "== DRY_RUN run_daily_omni.sh (MODE=${MODE}) =="
  echo "  MODE            : ${MODE}"
  echo "  DATA            : ${DATA}"
  ...
  echo "  benchmark_cmd   : python3 ${OFFICIAL_SCRIPT:-<official-harness>} --data ${DATA} --subset <official> --out ${OUT}（以官方 Harness 为准）"
...
# 官方脚本接入点。要求同脚本同子集同分母跑 baseline 与 candidate。
