...
**瓶颈发现**：T2W（Flow + Vocoder）在 CPU 上运行，占首音延迟 93%。

**cann-flow-only 发现**：`OMNI_T2W_DEVICE=cann-flow-only` 将 CANN backend 初始化推迟到 worker thread。T2W RTF 4.23→0.63（**6.7×**）。
...
**线程泄漏根因**：libgomp 为每个 httplib worker 创建 319-thread OpenMP team。`-t 4` 降至 3/session。5-6 session 后触发 cgroup pid 上限。

**WS Session 生命周期修复**：统一 finalizer，CTX_STATE_REUSABLE 正确重置。3 连续 E2E session 全部通过。
...
**Static Prefix KV Cache**：首次 save → 后续 load。Prefill p50 206ms→85ms（**2.4×** 加速比）。30 组 KV 完整性校验全部通过。

**LLM 量化 A/B**：Q8_0 RTF=0.565（−17.5% vs F16），0% LISTEN → **ACCEPT**。Q4_K_M 27-40% LISTEN → **REJECT**。

**Per-generation active + TTS KV guard**：消除跨请求 polling 竞争；prefill 阶段 cap 上限（256）。

**R13/S13 验证**：30/30 KV Cache A/B PASS；120/120 valid baseline，4 case types。

✅ KV Cache 2.4× | ✅ Q8_0 +17.5% | ✅ R13/S13 全部通过

### 第五阶段：Demo 准入与文本接口（8月5日–8月6日）

**Demo E2E**：Gateway→Worker→Backend 三层全链路验证。UTF-8 中文 30/30 PASS（L1 Backend 10/10 + L2 Worker 10/10 + L3 Gateway 10/10）。

**文本接口修复**：非流式 text 字段补齐、SSE crash（bad_alloc）修复、worker-once + sink.done guard。多模态 prefill 协议修正（media_type=2 user_text / think-loop 格式）。
...
**文档体系**：8 份顶层文档 2,276 行（Quickstart、架构、方法论、复现指南、证据索引、限制与 Gate）。

**比赛工具链**：RTF 解析器 + valid_audio 判定（10 种排除原因）+ Gate --dry-run（0/2/3/4）+ 私有路径清除。

**vLLM 迁移文档**：10 份文档在 `docs/vllm-migration/`。提交骨架：30 份文件在 `submission/`。

✅ 源码冻结 + 二进制复现 | ✅ 完整文档体系 | ✅ 比赛工具链就绪 | ⏳ 官方评测等待 Starter Kit
